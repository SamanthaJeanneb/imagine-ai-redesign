/**
 * The sign-in backdrop's renderer: a rose sky with soft clouds drifting
 * through it, drawn by a fragment shader at native resolution. Themed from
 * the live CSS tokens, and with a 2D gradient to fall back on where there is
 * no WebGL.
 */
...
/**
 * Starts the sky on a canvas and returns its teardown. With reduced motion it
 * paints one still frame and repaints only on a resize or a theme change.
 */
export function initCloudCanvas(
